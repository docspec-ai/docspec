...
describe("template", () => {
  ...
  describe("getFormatFilePath", () => {
    it("returns path to .docspec/docspec.md when it exists", async () => {
      await fs.mkdir(path.join(tempDir, ".docspec"), { recursive: true });
      await fs.writeFile(path.join(tempDir, ".docspec", "docspec.md"), "# Template", "utf-8");

      const formatPath = getFormatFilePath();

      expect(path.resolve(formatPath)).toBe(path.resolve(process.cwd(), ".docspec", "docspec.md"));
    });

    it("seeds .docspec/docspec.md from default when missing", () => {
      const formatPath = getFormatFilePath();

      expect(path.resolve(formatPath)).toBe(path.resolve(process.cwd(), ".docspec", "docspec.md"));
      const content = require("fs").readFileSync(path.join(process.cwd(), ".docspec", "docspec.md"), "utf-8");
