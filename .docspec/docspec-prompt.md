...
8. When you have made any documentation changes: create a new branch whose name starts with `docspec/` (e.g. `docspec/docs-sync`), commit your changes, push the branch, and open a pull request using the gh CLI (e.g. gh pr create). Using the `docspec/` prefix ensures the docspec-review workflow will not run again when this PR is merged. If you made no file changes, do not create a branch or PR.
