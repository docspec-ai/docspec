...
- **How do I use it?** - CLI commands (`docspec <filename.md>`, `docspec review`, `docspec generate`) and TypeScript API usage
...
- **Workflow files** (.github/workflows/docspec-review.yml, docspec-generate.yml): Workflow name changes, trigger changes, new steps or configuration
...
4. **Usage**: How to use docspec with subsections:
   - **CLI Commands**: Document default `docspec <markdown_path>`, `docspec review`, and `docspec generate` with examples matching src/cli.ts exactly
...
6. **GitHub Actions**: Prompt-only actions (docspec-review, docspec-generate); example workflow that runs docspec-review then Claude; no API keys required by docspec itself
...
- Workflow references should use actual file names (.github/workflows/docspec-review.yml, docspec-generate.yml)

**DO:**
- Use actual command examples: `docspec README.md`, `docspec generate README.md`, `docspec review --base X --merge Y`
