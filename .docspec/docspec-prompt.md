...
8. When you have made any documentation changes: create a new branch whose name starts with `docspec/` (e.g. `docspec/docs-sync`), commit your changes, push the branch, and open a pull request using the gh CLI. **IMPORTANT**: If a base branch is specified below, you MUST target that branch when creating the PR (e.g. `gh pr create --base <branch-name>`). If no base branch is specified, use the default branch. Using the `docspec/` prefix ensures the docspec-review workflow will not run again when this PR is merged. If you made no file changes, do not create a branch or PR.
